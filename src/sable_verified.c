...
TPM_STORED_DATA12 seal_passphrase(TPM_AUTHDATA srk_auth, TPM_AUTHDATA pp_auth,
  ...
  UINT32 seedLen = sizeof(TPM_NONCE) + sizeof(TPM_NONCE) + 3 + sizeof(TPM_SECRET);
  ...
  memcpy(seed+sizeof(TPM_NONCE)+sizeof(TPM_NONCE)+3, &sharedSecret, sizeof(TPM_SECRET));
  ...
  */
  ...
  struct TPM_Seal_ret seal_ret =
      TPM_Sealx(TPM_KH_SRK, encAuth, pcr_info, (const BYTE *) passphrase /*passEnc*/,
...
const char *unseal_passphrase(TPM_AUTHDATA srk_auth, TPM_AUTHDATA pp_auth,
  ...
  */
  ...
  sessions[0]->nonceOdd = get_nonce();
  sessions[0]->continueAuthSession = FALSE;
  TPM_SECRET sharedSecret = sharedSecret_gen(srk_auth, sessions[0]->osap->nonceEvenOSAP, sessions[0]->osap->nonceOddOSAP);
