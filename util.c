...
unsigned char kybrd_ctrl_read_status(void) { return inb(KYBRD_CTRL_STATS_REG); }
...
BYTE kybrd_enc_read_buf(void) { return inb(KYBRD_ENC_INPUT_BUF); }
...
int keyboardReader(BYTE *entry, UINT32 BufSize) {
  ...
  UINT32 t = 0;
  char c =
      key_stroke_listener(); // for some reason, there's always an 'enter' char
  while (t < BufSize) {
